...
// adly modified settimeout 500 -> 100
...
/*
    Adhly Add for fixed footer position after resolution change.
    function fixedScreen().
*/

function fixedScreen(){
/*
    fixed the panel height
*/
    var $displayarea = $(".page-content");
    var parentWidth = $('.main-content').height();
    var footer = $('.footer');

    //alert('page-content : '+$displayarea.height()+' + ' + 'footer' + footer.height()+ ' > ' +  'main-content : '+parentWidth);
...
function handlePanelAction() {
    ...
    function handlePanelControls() {
        ...
        $('.panel-controls-min').each(function() {
            var controls_html = '<div class="control-btn">' + '<a href="#" class="panel-toggle-min"><i class="fa fa-caret-square-o-down termOpen" id="termOpen"></i><i class="fa fa-window-close-o" id="termClose" style="display: none;"></i></a>' + '</div>';
            $(this).append(controls_html);
        });

        $('.panel-controls-min-show').each(function() {
            var controls_html = '<div class="control-btn">' + '<a href="#" class="panel-toggle-min-show"><i class="fa fa-caret-square-o-down termOpen" id="termOpen" style="display: none;"></i><i class="fa fa-window-close-o" id="termClose"></i></a>' + '</div>';
            $(this).append(controls_html);
        });
    }
    handlePanelControls();
    ...
    // Toggle Panel Content modified by adly
    $(document).on("click", ".panel-header .panel-toggle-min", function(event) {
        event.preventDefault();
        $(this).toggleClass("closed").parents(".panel:first").find(".panel-content").slideToggle();
        $(this).find(".fa").slideToggle();
        setTimeout(function(){
          fixedScreen()
        }, 250);
    });
    // Toggle Panel Content modified by adly
    $(document).on("click", ".panel-header .panel-toggle-min-show", function(event) {
        event.preventDefault();
        $(this).toggleClass("closed").parents(".panel:first").find(".panel-content").slideToggle();
        $(this).find(".fa").slideToggle();
        setTimeout(function(){
...
$('.nav-sidebar > li').hover(function() {
    ...
    //$(this).siblings().removeClass('nav-hover'); // adly modified
